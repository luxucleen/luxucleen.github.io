...
# TTS
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context and real-use notes. Covers list #49 as one unit: the audio engine under podcast/generate_podcast (#48) — Bella/Liam pairings identical everywhere.*
...
- Christian's go-to voices are the Meta AI pair: **Bella = `avocado_v2:MAI_01` (Warm)** and **Liam = `avocado_v2:MAI_03` (Smooth)** — reuse these exact ids for every briefing-style audio, and refer to them as Bella/Liam to the user, never by internal id.
- He is a content creator (**@djbrightfuture** — YouTube, Instagram, TikTok, Facebook, X): audio produced here is often headed to his socials. Default to `mp3` (the TTS default) so the file plays everywhere; ask before any other format.
- When TTS fails for an in-progress podcast or briefing, the retry ladder in Handling Failures below applies — never swap the Bella/Liam voices mid-series.
...
- No new hard-won lessons logged for this skill yet beyond what's already canon above (the retry ladder, and the id-verification step before retrying an opaque HTTP 500, both came from real use of this tooling). A voice copied verbatim from an authoritative source is a valid voice — a failure is not a reason to pick another one.
