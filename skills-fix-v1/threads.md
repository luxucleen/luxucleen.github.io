...
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context. Covers list #13 and #43 (Threads account and content) as one unit: reads, insights, and publish flows never fight because they share this file and the same write-safety rules.*
...
- `publish-post` is a write: it always goes through the skill's own confirmation flow first — Christian's standing "allows everything" means no extra approval gates are added on top, never that this gate is skipped.
- Free-traffic doctrine: content posted here exists to pull visitors to luxucleen.com — the destination page, maximum free/organic traffic, zero ad spend. The content engine (@djbrightfuture / @djbrightfutures on YouTube, Instagram, TikTok, Facebook, X) feeds this funnel.
- Luxu avatar canon: any avatar image or video published is the animated-WebP `<img>` loop living picture — never a `<video>` element; the Luxu keyboard must be identical in every asset; the status line under the name pill is blank when idle.
- Threads DMs live in the separate `threads_messages` skill — inbox, thread, and send flows are not here.
- Never poll from timers: rate limits are strict and Christian's watch machinery (15-min bridge28-watch, 5-min pingpong, 5-min christian-update-trigger) runs on its own crons — this skill reads on request.
