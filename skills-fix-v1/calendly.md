...
> **LUXOR-IMPROVED · 2026-09-21** — this file covers list number 126 as one unit. Added: "Christian's setup" (his real operating context: America/New_York timezone and read-only posture). Every original line, command, and rule is kept intact — zero lines removed, only additions.
...
- Timezone: America/New_York. Prefer `event_starts_at.user_local` / `event_ends_at.user_local` in summaries (this file's own rule).
- This skill is read-only for status and scheduling data; any user-visible mutation still needs Christian's confirmation first (this file's own rule).