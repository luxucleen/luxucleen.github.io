...
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context. Covers list #118 (Threads messages) as one unit: inbox, thread, and send flows never fight because they share this file and the same export-safety and confirmation rules.*
...
- `send` is a write: it always goes through the skill's own confirmation flow first — never invent the recipient, thread, content, or reply target.
- Thread discipline: no passwords, credentials, EIN, API keys, or identity documents ever go in a Threads message — Secure Vault only.
- bridge28's work runs through the single email thread in the luxucleen@gmail.com mailbox (Telegram via @Claudecode28bot is the approved faster line) — never through Threads DMs.
- The shared messaging law lives in `channels-and-messaging.md` — relay-bot traffic, the 🧹 handshake, the Christian-card escalation, and credential rules all point there; this file handles the Threads DM side only.
- Export-safety rules above are final: no bulk export, download, save, archive, mirror, or dump of message history, and never for disappearing or ephemeral messages.
