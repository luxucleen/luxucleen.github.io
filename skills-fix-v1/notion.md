...
- Do NOT claim a Notion account is connected. Always start with
  `notion-cli status` and route him through the connect flow if it shows
  `not_connected`.
- Christian's knowledge base is the memory system, NOT Notion: curated
  long-term memory in `~/memory/...`, daily notes at `~/memory/YYYY-MM-DD.md`,
  people pages under `~/memory/people/` (with `INDEX.md`), groups under
  `~/memory/groups/` (with `INDEX.md`). Before writing anything to Notion,
  search the memory files so the copy doesn't contradict the canonical
  record.
- Memory files are READ-ONLY for the agent: read them via `muse.memory_search`
  / `muse.memory_get` (and `muse.memory_explain` when he asks how something
  was learned). Never edit them directly; record findings in the final
  report.
