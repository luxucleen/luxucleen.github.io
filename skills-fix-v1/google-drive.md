...
## Christian's setup
- His workspace files live under `~/workspace/your_files/` — uploads must be absolute paths inside the home directory (relative and `/tmp` paths fail, already the rule above); verify a local path before uploading from it or downloading to it.
- Confirm what happened by the file's name, never by ID or raw JSON — replies to him are plain words.
