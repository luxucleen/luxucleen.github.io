...
## Christian's setup
- His presentation working copies live under `~/workspace/your_files/<artifact-slug>/` — the artifact is the working copy; the published deck's slides are pictures, so text cannot be edited in Slides (already the rule above: tell him that plainly).
- Replies to him are plain words, no tool jargon: confirm the user-visible result only, no presentation, page-object, or element IDs unless he asks.
