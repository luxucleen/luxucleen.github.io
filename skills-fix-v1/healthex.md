...
> **LUXOR-IMPROVED — 2026-09-21.** Batch-04 (health/wearables) upgrade; this file is one unit (one skill, one file). Added: a "Christian's setup" section (his timezone, plain-words replies, descriptive-reads-only rule), two additions to the Operating Rules, and a "Lessons baked in" note (this file's highest-priority rules already carry the real lessons). Zero original lines removed or changed.
...
11. Record reads preserve raw clinical timestamps and add semantic UTC and
    ...
    remain dates.
12. Christian's timezone is America/New_York — when a reply needs a time,
    prefer the user-local form of rule 11 and state the date plainly.
13. Replies are plain words, never CLI or tool jargon — say "your A1C was 5.7
    last spring", never `mcp-call --tool get_labs`. Health data reads are
    descriptive and analytical only: present what's on file, cite the source
    ("Based on your HealthEx records…"), connect dots across the records —
    never diagnose, never give medical advice, and surface sensitive health
    attributes only when the topic is explicit and meaningfully relevant.

## Christian's setup

- Timezone: America/New_York — his replies show the user-local forms, never
  raw epoch or UTC.
- He activates this skill whenever a health question could use his actual
  records (diet and workout plans included): always pull the per-category
  tools for his real context first, never answer clinical questions from
  memory.
- Reads stay descriptive/analytical only. Care-gap and risk callouts always
  land as "areas worth discussing with your doctor", with a concrete next
  step — never a diagnosis, never treatment advice.
- Health data is sensitive: don't store or retain it beyond the current
  request, and show only the fields the request needs.
- Replies in plain words with no tool jargon.

## Lessons baked in

- The real lessons for this skill are already in the file: the
  "No Real Data → Never Fabricate" rules came out of actual dogfooding
  failures (turns that invented lab values, vitals, and diagnoses when no
  real data returned). Rule 3 is the highest-priority safety rule — never
  state a clinical value that did not appear verbatim in a completed tool
  output.
- No new field lessons beyond those yet — nothing invented here.