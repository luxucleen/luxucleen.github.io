...
name: "muse-feedback"
title: "Muse feedback"
description: "Use for feedback and feature requests to the Muse team, including requests to join the general early access program. Whenever your response tells the user you can't do a specific thing they wanted, or accepts them giving up on one, offer once to file feedback in that response. This covers missing integrations you can't do yourself, capabilities you lack, and tasks that keep failing at a specific point. File only on their go-ahead. Also use when asked to send, view, check, or withdraw feedback."
metadata: { "includeInPrompt": true }
---

# Muse feedback

**LUXOR-IMPROVED — 2026-09-21** · Added Christian's go-ahead rule and the outward-facing-send discipline. Covers list unit: `muse-feedback` (1 file = 1 unit).
...
If the user explicitly asks to join Muse's general early access program,
use kind `missing-capability` and subject `early-access-group`, even when
a particular feature motivates joining.

Otherwise, interpret early-access and testing requests in the context of
the feature being discussed, including references to a beta, program, or waitlist.
Use that feature's ordinary kind and subject, include willingness to test
in the same report, and do not suggest joining the general program.

Follow the normal filing steps. When a request to join the program is marked
`addressed`, the team has added the user to the group.

The Muse team selects people from the group for early feature tests.
Do not promise feature access or testing priority.
...
- muse-feedback files only on Christian's go-ahead — never on inference, never bundled into other work. Draft first, ask once, file in a later turn after his explicit approval.
- Christian hates approval gates and allows everything, but feedback to the Muse team is an outward-facing send that carries his name, so it keeps its own go-ahead rule.

## Lessons baked in

- No real failure lessons from live use yet — this section stays short on purpose. The draft-first, file-later discipline is the safeguard: nothing reaches the Muse team without Christian seeing the exact words first.