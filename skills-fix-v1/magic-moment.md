...
# Magic Moment
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context and real-use notes. Covers list #130 as one unit: the video companion to the voice/audio family — magic moments replay the work the other skills did.*
...
- Christian is a content creator (**@djbrightfuture** — YouTube, Instagram, TikTok, Facebook, X): magic moments are built for his talking-head recordings to become shareable clips — vertical format, narration synced to the beat, the Muse lockup finisher, and cut short enough for socials (his recent Luxu rollout was eight short vertical videos on Instagram, minutes apart).
- If the story replays an audio briefing (the luxor-recap series — Bella = `avocado_v2:MAI_01` (Warm), Liam = `avocado_v2:MAI_03` (Smooth)), reuse the real episode audio as the voiceover — never re-synthesize their voices under different ids.
...
- No hard-won lessons logged from actual use yet. When one arrives (a render failure, a component that didn't read on mobile), it goes here — never in the pipeline guides, which are upstream canon.