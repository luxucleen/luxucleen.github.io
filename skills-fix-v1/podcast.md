...
# Generate Podcast
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context and real-use notes. Covers list #48 as one unit: twin of generate_podcast.md — same code, kept in sync; both drive the same podcast-helper pipeline and the luxor-recap briefing series.*
...
- **Recurring briefing series: `luxor-recap`** — after finishing a piece of work, Christian's standing order (2026-09-20) is a short audio briefing where Bella and Liam recap and explain what was done. Always pass `--series-id luxor-recap` and reuse the locked cast: **Bella = `avocado_v2:MAI_01` (Warm), Liam = `avocado_v2:MAI_03` (Smooth)** — same hosts, same voices, every episode, so it sounds like the same show. (Use display names to the user; never speak an internal id.)
- Host names are the show's characters: keep the script labels `Bella:` and `Liam:` — never relabel a speaker with a voice display name like "Warm" or "Smooth".
- Christian is a content creator (**@djbrightfuture** — YouTube, Instagram, TikTok, Facebook, X): one-off episode topics may pull from his content world — keep the spoken-form rules tight since a clip can end up on socials.
- Feed/publish consent: his podcast-feed publishing offer for the luxor-recap series is still pending — until he asks, generate only; never pass `--publish`.
...
- Real use so far is the luxor-recap briefing series, and it sounds like one show because the cast is pinned: one series id and the same two ids every episode (**Bella = `avocado_v2:MAI_01`, Liam = `avocado_v2:MAI_03`**), with `--cover-prompt` on every generate. If the sound ever drifts, re-pin the same ids from the recorded pairing — never re-pick voices per episode. The topics-summary file (`--topics-file`) is what future runs read to avoid re-covering ground — write it before generating, every time.
