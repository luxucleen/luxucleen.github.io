...
*LUXOR-IMPROVED 2026-09-21 — the full plan/execute/report workflow is untouched; added Christian's context on what his memory holds and where it lives. Covers list #135.*
...
- This skill acts on Christian's memory only on his explicit request ("forget that", "don't remember this about me", or a clear equivalent). "Forget it" as cancel-the-task never triggers it.
- His memory lives in `~/MEMORY.md`, the dated notes under `~/memory/`, and the people pages under `~/memory/people/` — all read-only day-to-day, all within reach of a forget plan.
- Standing orders (verification law, deploy hold, the bridge28 thread discipline, never-idle rule) are the operating core, not trivia — a forget request touching them gets the full plan + explicit confirmation flow, never a silent edit.
- The luxu-brain Cloudflare Worker, the avatar canon playbook, and the single bridge28 thread (`1a0c1b065b6618ac`) are business infrastructure: forgetting a fact about them does not delete the underlying files, which the planner must call out as a known limit.