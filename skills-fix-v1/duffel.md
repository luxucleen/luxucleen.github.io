...
> **LUXOR-IMPROVED · 2026-09-21** — this file covers list number 103 as one unit. Added: "Christian's setup" (his real operating context) and the travel-booking family conventions pointer. Every original line, command, flow, and rule is kept intact — zero lines removed, only additions.
...
or `ore_` ids to users. Do not show a Duffel command or ask the user to
paste payment-card, passport, traveler-security, loyalty, or account secrets
into chat. Run each search command directly into a distinct temporary JSON file;
...
5. Collect all required passenger details together. For each traveler, also
   load any applicable airline loyalty account already available through an
   authorized secure profile and include it in validation; do not ask them to
   re-enter a number that is already available.
...
Run exactly one Duffel `search` for each ordinary check. Search the complete
itinerary and passenger mix together with `--limit 30`. Do not use a browser or
web search for repricing. Retain only results that match the booked dated flight
numbers, airports, passenger count, cabin or fare family, included bags, stop
pattern, and material refund or change restrictions. Compute actionable net
savings after known cancellation, change, and rebooking costs.

Run a second fresh copy of the same search only when a comparable result appears
to cross an alert threshold. Require equivalent inventory to remain available
in the second result. Alert only when confirmed net savings exceed either 10
percent of the original paid total or the booking-currency equivalent of USD
100. Convert USD 100 with a fresh FX source and record the source time. If
reliable FX is unavailable, apply only the percentage threshold.
...
After offer selection, collect the following only through an approved secure
provider/profile/browser input surface:
...
On a text-only surface with no secure input, do not ask the user to paste
legal identity, birth date, contact, passport, traveler-security, loyalty, or
payment values into ordinary chat. Stop after the selected itinerary and price
are prepared, explain that secure traveler input is the blocker, and resume
when that surface is available. If the user volunteers a value in chat,
do not quote it back; acknowledge only that the corresponding field was
received and keep it out of later summaries.
...
When an applicable loyalty account is already known through an authorized
secure profile, use it automatically for the matching traveler in a native
booking. Add it to `loyalty_programme_accounts` before the first validation;
do not merely mention that it exists, omit it silently, or ask the user to
retype it. Match the program to the selected marketing or operating airline, or
to a supported partner program returned by validation. Do not infer an account
number from status, attach one traveler’s account to another traveler, or send
an unrelated program in the hope that it earns credit.

When several known programs are eligible and the best earning or benefit choice
is not clear, ask which one to credit. If the user said they have a program
but no secure number is available, ask for it only at checkout through the
secure provider input. Mention optional seat availability once without
blocking.

Validation verifies program support, attaches the account to the offer
passenger, refreshes pricing, and confirms it remained attached. Unknown,
unsupported, or failed attachment blocks checkout; omit it only with the
user's explicit agreement. This preparation has no HITL. Loyalty benefits
remain subject to the airline.
...
- Timezone: America/New_York. Every time shown to Christian is user-local; airport-local times are never converted without that airport's explicit timezone (this file's own presentation rule; the shared family promise lives in booking.md).
- Home base: Richmond, VA area. When a request implies "from home" and no origin airport is stated, confirm the airport with Christian — never assume one. A past trip is a search hint, not a standing origin.
- Long-term plan: move to Puerto Rico. Treat it as future travel context, never as the current origin or home anchor.
