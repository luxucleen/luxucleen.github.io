...
> **LUXOR-IMPROVED 2026-09-21** — Added "Christian's setup" (his real media-uploads context for photo work) and "Lessons baked in". Covers list numbers **70** as one unit. Every original line kept — only additions.
...
## Christian's setup
- His media uploads land in `~/workspace/user/media_library/` — that's the home directory the `path` field in results is relative to.
- He is a content creator — @djbrightfuture / @djbrightfutures (YouTube, Instagram, TikTok, Facebook, X) — so personal photo requests often ground his content work; lookups are cheap, so probe opportunistically and move on fast when nothing fits.
- Privacy matters for a creator: never surface his photos anywhere he didn't ask for, and keep identifiers (GPS, filenames) out of anything shared.
