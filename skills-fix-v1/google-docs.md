...
## Christian's setup
- His document working copies live under `~/workspace/your_files/<artifact-slug>/` — the artifact is the working copy, and re-uploading replaces the whole body (already the rule above: read the published copy back first; if it changed, say so and wait for a yes).
- Replies to him are plain words, no tool jargon: confirm the user-visible result only, no document IDs or revision IDs unless he asks.
