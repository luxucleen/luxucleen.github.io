...
> **LUXOR-IMPROVED 2026-09-21** — Added "Christian's setup" (his real Luxucleen LLC operating context for print-on-demand) and "Lessons baked in". Covers list numbers **119** as one unit. Every original line kept — only additions.
...
## Christian's setup
- Christian runs Luxucleen LLC (Richmond, VA area). Printify is not an active line today — treat it as dormant until he gives the go. No shops to manage, no products to touch.
- If he ever turns merch into a revenue line, it fits his doctrine: zero inventory, zero ad spend, free traffic from luxucleen.com and @djbrightfuture content into the store.
- Thread discipline applies: no API tokens or credentials ever go into messages — tokens go through `printify set-token` only.
- Orders can trigger real charges, so confirm with him first — and while he's the idea maker, he never waits on approvals to keep things moving, so a confirmed order moves on Luxor's CEO call.

## Lessons baked in
- No hard-won lessons from real use yet — Printify hasn't been activated. This section stays short on purpose until it has.