...
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context and hard-won lessons. Covers list #13 (account reads), #41 (posts), #42 (stories), #46 (insights), and #47 (publishing) as one unit: the five never fight because they share this file and the same account-and-rate rules.*
...
- Writes (`post-feed`, `post-story`, `set-profile-picture`, `update-bio`, interest updates) act only on Christian's explicit request and keep the skill's confirmation flow — his standing "allows everything" means he is never nagged, not that this gate is skipped.
- The 8-reel rollout (his standing order, 2026-09-21): 8 Luxu videos to the luxucleen Instagram account spaced ~8 minutes apart (roughly a one-hour rollout — his words: "Do not upload so fast. Wait eight minutes and then upload") — send him the links as they land.
- Free-traffic doctrine: Instagram (@djbrightfuture / @djbrightfutures) exists to pull visitors to luxucleen.com — the destination page, maximum free/organic traffic, zero ad spend.
- Luxu avatar canon: any avatar asset posted is the animated-WebP `<img>` loop living picture — never a `<video>` element; the Luxu keyboard must be identical in every asset; the status line under the name pill is blank when idle.
- luxucleen.com/orbits/ is do-not-touch — never edited or redesigned without his explicit say-so (this is a publishing skill, not a site-editing skill, but the rule stands if any posted design references it).
- Instagram DMs live in the separate `instagram_messages` skill — inbox, thread, search, react, and send flows are not here.
...
- The 8-reel rollout works on a cadence, not a flood: 8 Luxu videos ~8 minutes apart (~1 hour) on the luxucleen Instagram account — Christian's standing order after the first rollout (2026-09-21), "to make money once something is ready." Send him the links as they land.
