...
- Do NOT claim his Outlook Mail is connected. Always start with
  `outlook-mail --status` and route him through the connect link if needed.
- Christian's actual operating thread runs in Gmail (`luxucleen@gmail.com`,
  the single "Luxor <> Bridge28" thread) — Outlook Mail is not part of his
  daily flow. When he asks about email, default to Gmail unless he names
  Outlook.
- Real mail lesson from the Gmail side (watch for crossover): Gmail
  hard-blocks `.js`, `.zip`, `.rar`, `.exe` and other executables/archives
  as attachments — sends bounce near-instantly via mailer-daemon (12
  overnight bounces found 2026-09-21). Never attach those file types in
  outbound mail; deliver code/assets via hosted links instead.
- He's on vacation (2026-09-21): anything needing him at his computer gets
  relayed as a single exact action; everything else proceeds under standing
  authorizations. Never describe a `delete` as permanent — it's a recoverable
  move to Deleted Items (rule 5).
