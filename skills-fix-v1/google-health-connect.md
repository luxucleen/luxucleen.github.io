...
7. Replies to Christian are plain words, never CLI jargon — say "you walked
   8,214 steps today", never `query metrics --provider healthconnect`. Health
   data reads are descriptive and analytical only: report what's on file, spot
...
- Paired-device context comes from the device namespace (`device.list` shows
  the platform). This skill only has data when his paired device is Android —
  but never claim a specific device is paired or name devices he hasn't
  confirmed; check the live roster.
