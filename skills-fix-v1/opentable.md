...
> **LUXOR-IMPROVED · 2026-09-21** — this file covers list number 106 as one unit. Added: "Christian's setup" (his real operating context) and the travel-booking family conventions pointer. Every original line, command, flow, and rule is kept intact — zero lines removed, only additions.
...
- If an OpenTable command returns HTTP 429 or says it was rate limited, stop making OpenTable calls for this task and report the partial result. Do not retry in the same task. A later task can try again after the returned `retry_after`, or after 60 seconds if the response has no value.
...
- Timezone: America/New_York. Reservation reminder crons are resolved from the confirmed reservation time in this timezone (this file's own Book flow).
- Home base: Richmond, VA area — the anchor for "near me" restaurant searches when no device location is available.
- Restaurant-local civil times are never timezone-converted; show them labeled as local time (this file's own rule).
