...
icon: "facebook"
title: "Facebook"
description: "Use when the user provides a Facebook URL or asks to read personal posts, comments, reactions, friends, timelines, profiles, stories, feeds, groups, events, Marketplace listings, or saved items, or to create, edit, publish, or delete their Marketplace listings. For managed Facebook Pages, use post_coach instead."
metadata: { "includeInPrompt": true }
---

# Facebook CLI

*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context. Covers list #14 (personal Facebook ops) and #44 (Marketplace management) as one unit: the two never fight because they share this file and the same operating rules.*
...
- Marketplace writes (create, edit, delete, publish) keep the skill's approval prompt — the gate stays; a failed write is never silently retried (rule 19: surface the error and wait for his review). Saved-item add/remove may proceed from a clear request.
- Managed Facebook Pages belong to `post_coach` — never pass a Page ID into this skill; keep the boundary.
- Messenger traffic is the separate `messenger.md` skill (Companion) — messages on Messenger don't run through this file.
- Free-traffic doctrine: posts and Marketplace presence exist to pull visitors to luxucleen.com — the destination page, maximum free/organic traffic, zero ad spend.
...
│   ├── listing details (--listing-id <id> | --url '<item link>')  # Item details
...
├── events
│   ├── search [--scope connected|discover] [--keywords "..."] [--location "..."] [--category ...] [--start-date ...] [--end-date ...] [--limit N] [--after <cursor>]  # Paginated: data[] + paging.cursors.after
...
For any request about "my Facebook Page" or a named Page the user manages,
use `post_coach`, beginning with `social-insights post-coach linked-accounts`.
That result is the source of truth for managed Page profile details,
permissions, insights, posts, and publishing. Do not pass managed Page IDs to
`facebook-cli profile info` or `facebook-cli timeline fetch`.
...
- Reading a post, its comments, or its reactions (`post read`, `post comments read`, `post reactions read`)
- Resolving a Facebook share link (`link-sharing decode-url`) before choosing the content reader
- Reading the containing post for a canonical photo, video, or reel URL (`post read --url`)
- Viewing profile info (`profile info`)
- Browsing your newsfeed or friends feed (`feed newsfeed`, `feed friends`)
- Searching events or reading an event's details (`events search`, `events details`)
...
- **Managed Facebook Page -> Post Coach**: Use `social-insights post-coach linked-accounts` to resolve the Page and answer Page profile or permission questions; use Post Coach's dedicated Page commands for insights, posts, and publishing. Never treat its Page ID as a personal profile ID.
...
- **Groups search -> details/posts**: `groups search` produces `group_id` -> `groups details --group-id <id>` for group metadata or `groups posts --group-id <id>` for group posts
...
- **Events search -> details**: `events search` produces `id` (also embedded in `permalink_url`) -> `events details --event-id <id>` for the full event including `online_url`
...
| Posts and URLs | [posts.md](references/posts.md) | Decode share links, read posts by ID or canonical URL, or browse a timeline |
...
| Groups | [groups.md](references/groups.md) | Fetch group details, search groups, browse posts |
| Events | [events.md](references/events.md) | Search events, read event details |
...
21. **Keep managed Pages in Post Coach.** A Page ID returned by `social-insights post-coach linked-accounts` is not a person/profile ID for this skill. For Page identity, profile details, permissions, insights, posts, or publishing, use `post_coach` and its managed Page workflow instead of `facebook-cli profile info` or `facebook-cli timeline fetch`.