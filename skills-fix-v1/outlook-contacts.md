...
- Do NOT claim his Outlook contacts are connected. Always start with
  `outlook-contacts --status` and route him through the connect link if
  needed.
- His durable relationship records live in `~/memory/people/` (with
  `INDEX.md`), NOT in Outlook contacts. Before creating a contact for someone
  he knows (e.g. bridge28), check the people pages first so nothing in the
  contact contradicts the consolidated record. Example: `~/memory/people/bridge28.md`
  — his AI worker on luxucleen.com, run by Luxor as CEO.
- People and groups pages are read-only for the agent: read them via the
  memory tools, never edit them directly; note discrepancies in the final
  report.
