...
> **LUXOR-IMPROVED 2026-09-21** — Added "Christian's setup" (his real free-traffic operating context for images) and "Lessons baked in" (a real media-delivery lesson from actual use). Covers list numbers **23** as one unit. Every original line kept — only additions.
...
## Christian's setup
- Christian runs Luxucleen LLC (Richmond, VA area) and lives on the free-traffic doctrine: luxucleen.com is the destination, zero ad spend. Images found here feed that funnel — hero visuals, feed content, and reference art for @djbrightfuture content — so prefer locators that can ship (download-safe, per the rules above) over ones that only render in a chat preview.
- He opens deliverables on his iPhone: anything an Artifact ships with images in it must survive that context — no embeds that only work in a desktop browser.
- He is a content creator (@djbrightfuture / @djbrightfutures, YouTube, Instagram, TikTok, Facebook, X) — attribution matters for his work, so keep `page_url` for provenance on anything that ends up published.

## Lessons baked in
- catbox.moe hosting blocks data: URIs (learned 2026-09-21, real): its CSP refuses `data:` URIs in img-src/media-src. When shipping a page that shows images hosted on catbox, upload each media file as its own separate file and reference it by URL — never inline it as a data: URI. The page itself still serves fine as text/html with inline scripts.