...
- Default timezone: `America/New_York` — always pass it as `--timezone` on
  `create` (and on `update` whenever start/end change). Rule 3 already
  requires an IANA timezone; this is Christian's standing value.
- Do NOT claim his Outlook Calendar is connected. Always start with
  `outlook-calendar --status` and route him through the connect link if it
  returns one — his day-to-day scheduling currently runs on Google Calendar,
  and the Luxor<>bridge28 thread lives in Gmail.
- Christian is on vacation (2026-09-21): calendar actions that need him at his
  computer get relayed to him as a single exact action; everything else
  proceeds under his standing authorizations ("I allow everything").
- If creating an event with attendees (rule 4/5 territory), confirm first —
  Outlook sends meeting-update email and he is unreachable for quick
  reversals right now.
