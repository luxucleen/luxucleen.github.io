...
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context and hard-won lessons. Covers list #1 (search/read) and #2 (draft/send) as one unit: the two never fight because they share this file and the same Rules.*
...
- Default working mailbox: luxucleen@gmail.com.
- The bridge28 thread — subject "Luxor <> Bridge28 — working arrangement", thread id `1a0c1b065b6618ac`. Read it with `users threads get --params '{"userId":"me","id":"1a0c1b065b6618ac","format":"full"}'`. Collect messages newest-first; track the newest message id as a watermark so each check only reads what's new.
- Watch pattern: poll the thread on a timer, report only what matters (STAGED/LIVE/DONE/BLOCKED, questions, blockers). Routine acknowledgments are not worth a message.
- Thread discipline: never send mail from a watch job unless the job's own instructions order it. Never start new threads. Never put passwords, credentials, EIN, or identity documents in email — Secure Vault only.
- After any send with attachments, scan the inbox for mailer-daemon bounces — Gmail's policy rejections are near-instant and land UNREAD.
...
- List candidates with their sender, subject, and date: `+triage --query '<query>' --max 50 --format json`. Triage first, read second — never open full bodies to find candidates.
...
- Download an attachment (resolve a concrete message id and attachment id first): `users messages attachments get --params '{"userId":"me","messageId":"<id>","id":"<attachment_id>"}'` returns the file as base64url in a `data` field. Decode that `data` to the output path yourself. `--output` does not write it. The path `id` param takes the ATTACHMENT id, not the message id — a bare `id`=<message> errors. Attachment tokens ROTATE on every `messages get`: never reuse a token from an earlier fetch; re-fetch the message fresh right before downloading.
...
- Attachment downloads: the path `id` is the ATTACHMENT id, never the message id; tokens rotate on every `messages get`, so re-fetch fresh right before downloading. (Found the hard way 2026-09-21.)
- Never attach `.js`, `.zip`, `.rar`, `.exe` or other executables/archives — Gmail bounces them via mailer-daemon. Deliver code and assets through hosted links instead. After any send with attachments, scan the inbox for mailer-daemon bounces.
- Reading and writing share this one file and the same Rules below — that is what keeps #1 and #2 from ever fighting: one skill, one set of laws, read flows and write flows side by side.
