...
> **LUXOR-IMPROVED · 2026-09-21** — this file covers list number 105 as one unit. Added: "Christian's setup" (his real operating context) and the travel-booking family conventions pointer. Every original line, command, flow, and rule is kept intact — zero lines removed, only additions.
...
- Do not ask the user to paste payment-card, passport, traveler-security, or
  loyalty numbers into chat. Collect sensitive values only through an approved
  secure provider, profile, wallet, credential, or browser input surface, and
  do not repeat them in a message.
...
- **Sensitive booking identity:** legal name, date of birth, passport details,
  ...
  and payment credentials. Read or save these only through an approved secure
  profile, wallet, or provider connector. Do not put them in ordinary memory,
  notes, artifacts, or a chat message. If no secure store exists, use them only
  for the active checkout and do not claim they were remembered.
...
- Timezone: America/New_York. Resolve relative dates ("tomorrow", "next Tuesday") to exact local dates in this timezone before searching.
- Home base: Richmond, VA area. "Near me" / "near home" anchors here when no device location is available and the request implies home — never invent a specific airport, hotel, or restaurant.
- Long-term plan: move to Puerto Rico. Future context for trip planning, never the current origin.
...
- No booking profile facts are invented: an explicit statement is a preference; a repeated pattern is tentative; one old booking is only a search hint (this file's own evidence rules).

## Booking-family conventions

Shared with duffel.md, flightaware.md, opentable.md, places-search.md, ticketmaster.md, travel-planning.md (each of those files points back here — the details live in one place so the skills never fight each other):

- **Times:** every time shown to Christian is user-local, America/New_York, unless the item is venue/airport-local (flight gates, restaurant tables, event doors) — then show it labeled in that local time and never convert it without that place's explicit timezone.
- **Locations:** home base = Richmond, VA area; long-term plan = Puerto Rico. Use Richmond as the home anchor only when the request implies it; never invent a specific airport or venue.
- **Verification:** for any flight question, verify the exact dated flight first — memory may identify the itinerary but never verifies its current schedule.
- **Money honesty:** full amounts including taxes and mandatory fees on every shortlist; label estimates as estimates; never quote a price from memory — re-run the live check.
- **Secrets stay out of chat:** card, passport, identity, and loyalty numbers move only through secure surfaces and are never repeated in a message.