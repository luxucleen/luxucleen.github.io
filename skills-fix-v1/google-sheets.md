...
## Christian's setup
- Christian's timezone is America/New_York — dates in replies are grounded there; `valueInputOption: USER_ENTERED` lets Sheets parse dates and currency in his locale.
- To give him a workbook to download, build a spreadsheet artifact under `~/workspace/your_files/<artifact-slug>/` — never publish an `.xlsx` over his spreadsheet (already the rule above).
- Replies to him are plain words, no tool jargon: no spreadsheet or sheet IDs, no page tokens, no raw JSON.
