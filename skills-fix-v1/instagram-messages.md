...
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context and hard-won lessons. Covers list #117 (Instagram messages) as one unit: inbox, thread, search, react, and send never fight because they share this file and the same confirmation rules.*
...
- `send` and `react` are writes: they always go through the skill's own confirmation flow first — never invent the recipient, thread, content, message, or reaction.
- Thread discipline: no passwords, credentials, EIN, API keys, or identity documents ever go in an Instagram DM — Secure Vault only.
- Relay-bot traffic never runs through Instagram DMs — it goes through the no-browser `telegram_relay` pipeline (vaulted credential, Bot API); bridge28's work runs through the single email thread in the luxucleen@gmail.com mailbox.
- The shared messaging law lives in `channels-and-messaging.md` — relay-bot traffic, the 🧹 handshake, the Christian-card escalation, and credential rules all point there; this file handles the Instagram DM side only.
- Export-safety rules above are final: no bulk export, download, save, archive, mirror, or dump of message history, and never for disappearing, view-once, vanish-mode, or ephemeral messages.
...
- Never send a demo as an `.html` DM attachment — the iPhone file viewer does NOT run JavaScript, so the whole JS-driven surface renders blank (found with the luxu chat demo, 2026-09-21). Host the page and send the link instead.
