...
> **LUXOR-IMPROVED 2026-09-21** — Added "Christian's setup" (his real operating context for product questions) and "Lessons baked in". Covers list numbers **52, 53** as one unit. Every original line kept — only additions.
...
## Christian's setup
- Christian runs Luxucleen LLC (Richmond, VA area); he is a content creator — @djbrightfuture / @djbrightfutures (YouTube, Instagram, TikTok, Facebook, X). Gear requests (camera, streaming, audio gear) are the most likely shopping use; resolve required attributes against his durable shopping profile before searching.
- Free-traffic doctrine: luxucleen.com is the destination, zero ad spend. Shopping work here is about saving him money, not spending it — the "aim to save the user money" line above is his standing rule.
- Use this skill first for any product question (standing routing order), before browser search or general research.
- Purchases are his call; card details never in chat, thread discipline holds — wallet setup follows the fast-checkout rules, money reports stay in the checkout currency, never converted silently.

## Lessons baked in
- Results as Markdown files don't land on his iPhone (learned 2026-09-21): the iOS attachment viewer renders no JavaScript and he opens files there, so deliver shopping results as widgets or hosted page links, never as .md/.html attachments.