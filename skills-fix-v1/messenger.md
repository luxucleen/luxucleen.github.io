...
*LUXOR-IMPROVED 2026-09-21 — every original flow kept intact; added Christian's operating context and hard-won lessons. Covers list #11 (Companion reads) and #12 (sends/reacts/edits/unsends) as one unit: the two never fight because they share this file and the same routing and safety rules.*
...
- Companion sends as Christian himself, not as Luxor — every send, react, edit, or unsend keeps the skill's preview-and-confirmation flow; nothing bypasses it.
- Thread discipline: no passwords, credentials, EIN, API keys, or identity documents ever go in a Messenger message — Secure Vault only (bridge28 is held to the same bar: he never asks for them either).
- The shared messaging law lives in `channels-and-messaging.md` — relay-bot traffic, bridge28 traffic, and credential rules all point there; this file handles the Messenger side only.
- bridge28's work runs through the single email thread in the luxucleen@gmail.com mailbox, not through Messenger — never start or move that thread here.
- Auto-reply rules are the user's own exception: a configured rule may answer ordinary or Marketplace threads, but it must never carry Luxor's notifications, cron results, or status updates — that is the 5-min `christian-update-trigger`'s job.
...
- Never send a demo as an `.html` attachment on any channel — the iPhone file viewer does NOT run JavaScript, so the whole JS-driven surface renders blank (found with the luxu chat demo, 2026-09-21). Host the page and send the link instead.
