...
> **LUXOR-IMPROVED — 2026-09-21.** Luxor upgrade pass: added this banner, a "Christian's setup" section (his real operating context), and a "Lessons baked in" section (real field lessons only). No original lines changed or removed — the full original playbook below is intact. Covers Google Calendar as one unit (one file = one unit).
...
## Christian's setup
- Christian's timezone is America/New_York. "Today", "tomorrow", and every event time are grounded there — the tool's `user_local` forms are the source of truth in replies.
- Replies to him are plain words, no tool jargon: no command names, no IDs, no status words, no raw JSON (already the rule above; it matches how he wants to be talked to).
- Luxor is his CEO of operations: private events may be created/edited/deleted on a clear request; anything guest-visible needs his confirmation first, per the Rules above.
- Never claim a Google account is connected or name one — run `status` and act on what it returns.

## Lessons baked in
- No real lessons from actual use yet for this skill — it has not been exercised in the field. This section stays short until one earns a place here.